#this is a slightly altered version of the program to scrape search results designed to be used
#with a cron job initiation on a daily basis for a variety of search terms.
...
#rather than take user input, take command line inputs from the cron job
input = ARGV[0]
...
#THIS CODE IS COMMENTED OUT TO REFRAIN FROM ASKING FOR USER INPUT
#check to see whether or not the user wants to update the database with the results and if they do, update.
#db_update?(date, input, serps, url_array)

#THIS CODE AUTOMATICALLY UPDATES THE DATABASE WITH NO USER INPUT REQUIRED.
update_db(date, input, serps, url_array)

#the cron job for this runs at 9am every morning (in theory) and automatically updates the database
